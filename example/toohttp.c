...
/*
    代码中除了用到 C
   语言标准库的一些函数，也用到了一些与环境有关的函数(例如POSIX标准)
    具体可以参读《The Linux Programming
   Interface》，以下简称《TLPI》，页码指示均为英文版
...
void accept_request(void *arg) {
  ...
  // 将前面分隔两份的前面那份字符串，拼接在字符串htdocs的后面之后就输出存储到数组
  // path 中。相当于现在 path 中存储着一个字符串
...
int main(void) {
  ...
  pool = threadpool_create(1, 10, 20);
